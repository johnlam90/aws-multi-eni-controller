...
func (r *NodeENIReconciler) createAndAttachENIForSubnet(ctx context.Context, nodeENI *networkingv1alpha1.NodeENI, node corev1.Node, instanceID string, subnetID string, deviceIndex int) error {
	...
	eni, err := r.AWS.DescribeENI(ctx, eniID)
	if err != nil {
		log.Error(err, "Failed to get subnet CIDR, continuing without it", "eniID", eniID)
	} else if eni != nil {
...
func (r *NodeENIReconciler) verifyENIAttachments(ctx context.Context, nodeENI *networkingv1alpha1.NodeENI, nodeName string) {
	...
	for _, attachment := range nodeENI.Status.Attachments {
		...
		if eni.Attachment != nil && eni.Attachment.AttachmentID != attachment.AttachmentID {
			...
			// Update the attachment with the new attachment ID and subnet CIDR if available
			updatedAttachment := attachment.DeepCopy()
			updatedAttachment.AttachmentID = eni.Attachment.AttachmentID
			// Update the subnet CIDR if it's available and not already set
			if eni.SubnetCIDR != "" && updatedAttachment.SubnetCIDR == "" {
				updatedAttachment.SubnetCIDR = eni.SubnetCIDR
			}
